...
│  └─ utils.py
├─ scripts/
│  └─ create_hf_dataset.py   # builds the HF dataset (incl. Davidson) with cleaning & 80/10/10 split
├─ configs/
...
├─ outputs/                  # checkpoints & prediction files (example checkpoints only)
├─ requirements.txt
├─ setup.py
└─ README.md
```

Label encoding used throughout: `0 = hateful, 1 = offensive, 2 = clean` (Davidson et al.'s original encoding).
