...
from transformers import set_seed, AutoModelForSequenceClassification, AutoTokenizer #AK added autotokenizer
...
"""
chall.py was used for evaluating the checkpoints on HateCheck-XR (Check run_all_test_challenge.py for more)
"""
...
def evaluate_data(
    ...
    """Function for running model on evaluation set.
...
#NEW by AK
    ...
    raw = load_dataset(
        "csv",
        data_files={"test": ["dataset/extended_hatecheck/reannotation.csv"]},
        delimiter=";",
        encoding="utf-8-sig",
        download_mode="force_redownload"
    )["test"]

    #Map columns into 'text' and integer 'labels', and ADJUST depending on types of experiments!


    #1. 3 classes. Use the following line and unify but not more
   # label_map = {"hateful": 0, "offensive": 1, "clean": 2} # 3 classes & Hate vs Clean (for 3 classes: don't use new functions)

    label_map = {"hateful": 0, "offensive": 0, "clean": 1} # Notclean vs Clean
   # label_map = {"hateful": 0, "offensive": 1, "clean": 1} # Hate vs Non-hate

    def unify(ex):
        return {
            "text":   ex["test_case"],                             
            "labels": label_map[ex["label_gold"]]               
        }
    ...
    # --2. only test on Hate vs Clean (drop offensive) ---

    # 1) DROP offensive examples in the csv
    #ds = ds.filter(lambda ex: ex["labels"] != 1)

    # 2) REMAP labels: 0->0 (hate stays the same), 2->1 (clean becomes 1)
  #  def merge_labels(ex):
...
# -----------------------------
    ...
    ds = ds.map(lambda x: tokenizer(x["text"].lower(),
                                    ...
                                    padding=padding),
                batched=False) #same settings as test.py, and from hs.utils
    
    ds.set_format(type="torch", columns=["input_ids","attention_mask","labels"])

    dataset, tokenizer = ds, tokenizer
