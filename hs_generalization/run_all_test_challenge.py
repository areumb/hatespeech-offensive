...
#seeds = [ 8, 94, 300, 1024, 2111] #hate-nonhate on hate-nonhate ------------change & SELECT seed list each time!
seeds= [7, 222, 550, 999, 3111] #hate clean: for loading pre-trained model
...
for seed in seeds:
    ...
    for ckpt in checkpoint_files:
        ...
        subprocess.run([
            "python", "hs_generalization/chall.py",
            "-c", config_path,
            "--seed", str(seed),
            "--checkpoint", ckpt
        ],
            check=True,
            stdin=subprocess.DEVNULL,   # <- no stdin, so no press Enter needed anymore
        )
        print() 
